...
namespace AngelBroking
{
    ...
    public class SmartApi
    {
        ...
        public void ConnectAngelWebsocket(Dictionary<string, string> headers)
        {
            // Connected to Angel API. Now Code for connecting to Websocket starts here
            ...
            try
            {
                // Connect to the web socket using the URL and the cancellation token
                WebSocket.Connect(ws_url, headers);//, cts.Token);

                // The connection is successful, you can send and receive data here
                //Console.WriteLine("websocket connected");
                // ...
            }
            catch (WebSocketException ex)
            {
                // The connection failed, check the error message
                string errorMessage = ex.Message;

                // If the error message contains the x-error-message header, extract it
                if (errorMessage.Contains("x-error-message"))
                {
                    // Find the index of the header value
                    int index = errorMessage.IndexOf("x-error-message") + 16;

                    // Get the substring from the index to the end of the line
                    errorMessage = errorMessage.Substring(index).TrimEnd('\r', '\n');
                }

                // Display the error message on the console
                Console.WriteLine("WebSocket connection failed: " + errorMessage);
            }
        ...
        public void Subscribe(string correlationID, int action, int mode, List<Exchange> tokenList)
        {
            var request = new
            {
            ...
            //byte[] request_bytes = Encoding.UTF8.GetBytes(request_json);
            //var buffer = new ArraySegment<byte>(request_bytes);
        ...
        // Create a class to represent the exchange
